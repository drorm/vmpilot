...
def _modify_state_messages(state: AgentState):
    ...
    logger.info(f"modifying state messages: {state}")
        ...
        if cached > 0:
            ...
            if isinstance(message.content, list):
                for block in message.content:
                    ...
                    logger.info(f"Added cache_eph block: {block}")
                    cached -= 1
            else:
...
async def process_messages(
    *,
        ...
        for msg in messages:
            ...
            if msg["role"] == "user":
                if isinstance(msg["content"], str):
                    ...
                    logger.info(f"Processing list of messages")
    ...
    async def process_stream():
        # logger.info(f"Processing stream with messages: {formatted_messages} ")
        try:
