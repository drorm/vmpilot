...
for i in {1..3}; do
    ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" "$SCRIPT_DIR/run_cli.sh" -t 0 "echo 'Request $i'" > "$TEST_DIR/output_$i.txt" 2>&1 &
done

# Wait for all background processes
wait

# Add a small delay to ensure files are written completely
sleep 1

# Check all outputs exist and are valid
for i in {1..3}; do
    if [[ -f "$TEST_DIR/output_$i.txt" ]] && grep -q "Request $i" "$TEST_DIR/output_$i.txt"; then
        echo "✓ Concurrent request $i completed successfully"
    else
        echo "✗ Concurrent request $i failed"
        echo "Content of output_$i.txt (if exists):"
        [[ -f "$TEST_DIR/output_$i.txt" ]] && cat "$TEST_DIR/output_$i.txt" || echo "File not found"
        exit 1
    fi
done
...
response=$(ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" "$SCRIPT_DIR/run_cli.sh" -t 0 "sleep 5 & echo 'immediate'" 2>&1)
...
response=$(ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" "$SCRIPT_DIR/run_cli.sh" -t 0 "echo 'start' && sleep 2 && echo 'end'" 2>&1)
...
# response=$(ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" "$SCRIPT_DIR/run_cli.sh" -t 0 --stream "for i in {1..3}; do echo \$i & done" 2>&1)
