...
for i in {1..3}; do
    ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" "$SCRIPT_DI"$SCRIPT_DIR/run_cli.sh"" -t 0 "echo 'Request $i'" > "$TEST_DIR/output_$i.txt" 2>&1 &
done
...
response=$(ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" "$SCRIPT_DI"$SCRIPT_DIR/run_cli.sh"" -t 0 "sleep 5 & echo 'immediate'" 2>&1)
...
response=$(ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" "$SCRIPT_DI"$SCRIPT_DIR/run_cli.sh"" -t 0 "echo 'start' && sleep 2 && echo 'end'" 2>&1)
...
# response=$(ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" "$SCRIPT_DI"$SCRIPT_DIR/run_cli.sh"" -t 0 --stream "for i in {1..3}; do echo \$i & done" 2>&1)
