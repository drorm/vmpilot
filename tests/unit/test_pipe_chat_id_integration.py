    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                while time.time() - start_time < 1:  # Wait up to 1 second for threads to complete
...
class TestPipeChatIDIntegration:
    ...
    def test_message_truncation_with_chat_id(self, mock_process_messages, pipeline):
        ...
        with patch('queue.Queue.get', get_mock), patch('asyncio.new_event_loop', return_value=mock_loop):
            ...
            with patch('threading.Thread', fake_thread):
                ...
                if mock_process_messages.call_args is None:
                    pytest.fail("The process_messages method was never called - test timed out")
                else:
