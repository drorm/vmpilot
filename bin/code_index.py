#!/usr/bin/env python
"""
Command-line script for building the code index.
This script directly imports the necessary modules and handles all Python path issues.
"""
...
# Import the create_code_index function
try:
    from vmpilot.ragcode.indexer import create_code_index

    logger.info("Successfully imported create_code_index from vmpilot.ragcode.indexer")
    ...
    try:
        ...
        logger.info(
            "Successfully imported create_code_index from ragcode.indexer (old location)"
        )
    except ImportError:
        logger.error(f"Error importing from ragcode.indexer: {e}")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=150,
        help="Maximum size of code lines (default: 150)",
    )
    parser.add_argument(
        "--chunk_lines_overlap",
        type=int,
        default=75,
        help="Overlap between consecutive chunks (default: 75)",
    )
